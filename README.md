...
正式上传前必须在已启动并登录 Photoshop 的 Windows 交互桌面运行 `tests/windows_release_e2e.ps1`。脚本会从最终 ZIP 全新解压，检查启动脚本编码与换行、随包 Python 固定版本，并从真实用户入口完成一次单商品 JPG/PSD 导出；只有结果报告无阻断错误、成品签名与任务记录齐全时才会通过。
